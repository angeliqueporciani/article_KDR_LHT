...
glm_NR_kdr <- glmmTMB(passage_ny1phe~nom+(1|replicat), data=LHT_mod, family=binomial(link="logit"))
#summary(glm_NR_kdr)

emm_NR_kdr <- emmeans(glm_NR_kdr, pairwise~nom, infer=TRUE, type="response")

emm_NR_kdr$contrasts %>% kbl(caption = "Hatching rate comparisons", booktabs = T)%>%
	kable_styling(full_width = F)
...
```{r}
LHT_mod %>%dplyr::group_by(nom) %>% dplyr::summarise(medianTP=median(jour, na.rm=TRUE))
```
...
BFR_data_plot %>%drop_na() %>%  ggplot()+
 geom_point(aes(x=nom, y=prop_gorgee, colour=nom))+
